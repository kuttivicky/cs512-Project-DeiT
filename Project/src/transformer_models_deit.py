...
class PatchEmbedding(nn.Module):
    def __init__(self, in_channels=3, patch_size=4, emb_size=64, img_size=32):
        ...
        self.distill_token = nn.Parameter(torch.randn(1, 1, emb_size))
        self.positional_embedding = nn.Parameter(torch.randn(1, (img_size // patch_size) ** 2 + 2, emb_size))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x).reshape(batch_size, tokens, 3, self.num_heads, emb_size // self.num_heads)
...
class DistilledVisionTransformer(nn.Module):
    def __init__(self, in_channels=3, patch_size=4, emb_size=64, img_size=32, num_classes=10, depth=7, num_heads=8, forward_expansion=4, dropout=0.1):
        super().__init__()
        ...
        self.head_cls = nn.Linear(emb_size, num_classes)
        self.head_distill = nn.Linear(emb_size, num_classes)

    def forward(self, x):
        x = self.patch_embedding(x)
        x = self.transformer_encoders(x)
        x = self.layernorm(x)
        cls_logits = self.head_cls(x[:, 0])  # Classification token
        distill_logits = self.head_distill(x[:, 1])  # Distillation token
